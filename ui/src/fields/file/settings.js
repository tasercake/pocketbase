...
export function settings(data) {
	...
	const hdrThumbsPolicyOptions = [
		{ label: "Off", value: "off" },
		{ label: "Require HDR output", value: "require" },
	];

	const normalizeHdrThumbsPolicy = () => {
		if (!data.field.hdrThumbs || !data.field.hdrThumbsPolicy) {
			data.field.hdrThumbsPolicy = "off";
		}
	};

	normalizeHdrThumbsPolicy();
		...
		content: () =>
			t.div(
				...
				t.div(
					...
					t.div(
						...
						app.components.select({
							...
							onchange: (opts) =>
								(data.field.mimeTypes = opts.map((opt) => opt.value)),
						}),
					),
					t.div(
				...
				t.div(
					{
						className: () =>
							data.field.maxSelect > 1 ? "col-sm-6" : "col-sm-9",
					},
					t.div(
						{ className: "field" },
						t.label(
						...
						t.input({
							...
							onchange: (e) =>
								(data.field.thumbs = app.utils.splitNonEmpty(
									e.target.value,
									",",
								)),
						}),
				...
				t.div(
					...
					t.div(
						...
						t.input({
							className: "switch",
							type: "checkbox",
							id: uniqueId + ".hdrThumbs",
							name: () => `fields.${data.fieldIndex}.hdrThumbs`,
							checked: () => !!data.field.hdrThumbs,
							onchange: (e) => {
								data.field.hdrThumbs = e.target.checked;
								normalizeHdrThumbsPolicy();
							},
						}),
						t.label(
							{ htmlFor: uniqueId + ".hdrThumbs" },
							t.span(
								{ className: "txt" },
								"Generate HDR thumbnails for HDR images",
							),
							t.small(
								{ className: "txt-hint" },
								"Preserve HDR colors for generated image thumbnails.",
							),
						),
					),
				),
				t.div(
					...
					t.div(
						{ className: "field" },
						t.label(
							{ htmlFor: uniqueId + ".hdrThumbsPolicy" },
							"HDR thumbs policy",
						),
						app.components.select({
							id: uniqueId + ".hdrThumbsPolicy",
							name: () => `fields.${data.fieldIndex}.hdrThumbsPolicy`,
							options: hdrThumbsPolicyOptions,
							value: () => {
								normalizeHdrThumbsPolicy();
								return data.field.hdrThumbsPolicy;
							},
							disabled: () => !data.field.hdrThumbs,
							onchange: (opts) => {
								data.field.hdrThumbsPolicy = opts?.[0]?.value || "off";
								normalizeHdrThumbsPolicy();
							},
						}),
					),
					t.div(
						{ className: "field-help" },
						"When HDR thumbs are disabled, the policy is normalized to off. Require makes HDR thumbnail generation fail if HDR cannot be preserved.",
					),
				),
				t.div(
					{ className: "col-sm-12" },
